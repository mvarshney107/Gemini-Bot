...
genai.configure(api_key="GEMINI API KEY")  # or replace with relevant gemini api key
model = genai.GenerativeModel('gemini-1.5-flash', 
    safety_settings = [  # temporary measure, safety settings have tendency to cause errors
        {
...
def respond_with_limit(prompt):
    response = model.generate_content(f"Answer this following prompt in 2000 characters or less (this is a hard limit and cannot be exceeded): {prompt}")
    return response.text


def contextual_response(prompt, context):
    response = model.generate_content(f"Answer this following prompt in 1000 characters or less (this is a hard limit and cannot be exceeded): {prompt}. Here is the message history of this particular conversation for context (there may be none): {context}")
    return response.text
...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.DMChannel):  # if user sends a direct message
        ...
        if message.reference:  # if the message is a reply
            ...
            if message.content not in server_context[user_id]:
...
bot.run("DISCORD API KEY")  # or replace with relevant discord bot key
